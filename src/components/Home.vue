<template>
  <el-upload class="avatar-uploader"
    ...
    <h1 v-text="msg" ></h1>
    <el-input style="margin-left: 20px;" v-model="imageName" placeholder="请输入名称"></el-input>
<!--    日期格式化:{{c_time}}-->
  ...
  methods: {
    ...
    uploadURL() {
      for(var i =0;i<this.fileList.length;i++){
        let fileName = `${this.imageName}` + `${this.fileList[i].raw.uid}` + '.jpg'; //定义唯一的文件名
        ...
        formData.append("imageUrl", 'https://lqhwechat.oss-cn-hangzhou.aliyuncs.com/' + fileName);
        formData.append("imageName", this.imageName);
        if (this.c_time != '') {
          this.axios.post('/api/image/add', formData).then(function () {
            alert('上传成功');
          }).catch(function (e) {
            alert(e)
          })
